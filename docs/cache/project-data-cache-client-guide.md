# Project Data Access API - HTTP 캐싱 클라이언트 가이드

## 개요

Project Data Access API는 **HTTP 캐싱 (ETag)**을 활용하여 성능을 최적화합니다.
...
- ✅ **네트워크 절약**: 304 응답 시 body 없음 (71.3% 응답 시간 단축)
...
### **시나리오 1: 일반적인 Study 목록 조회**

브라우저가 자동으로 캐싱 처리.

```javascript
// 첫 번째 요청 - 200 OK, ETag 저장
const response1 = await fetch('/api/project-data/634/studies', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
const studies1 = await response1.json();
console.log('ETag:', response1.headers.get('ETag')); // W/"1737734400"

// 60초 내 두 번째 요청 - 브라우저 캐시 사용 (0ms)
const response2 = await fetch('/api/project-data/634/studies', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
...
const response3 = await fetch('/api/project-data/634/studies', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
...
### **시나리오 2: Study 할당 후 즉시 조회**

Study를 프로젝트에 할당한 후 즉시 최신 데이터를 조회해야 하는 경우.

#### ❌ **잘못된 방법** (캐시된 데이터 반환 가능)

```javascript
// Study 할당
await fetch('/api/project-data/634/studies/1', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${token}`,
    'Content-Type': 'application/json'
...
const response = await fetch('/api/project-data/634/studies', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
// ❌ 새로 할당한 Study가 목록에 없을 수 있음!
```
...
const studies = await response.json(); // ✅ 최신 데이터 보장
```
...
async function getProjectStudies(projectId, forceRefresh = false) {
  const headers = {
    'Authorization': `Bearer ${token}`
  };
  ...
  const response = await fetch(`/api/project-data/${projectId}/studies`, { headers });

  if (response.status === 304) {
    console.log('데이터 변경 없음 - 캐시 사용');
    return cachedStudies; // 캐시된 데이터 반환
  }

  if (response.ok) {
    cachedETag = response.headers.get('ETag');
    cachedStudies = await response.json();
    console.log('새 데이터 수신 - ETag:', cachedETag);
    return cachedStudies;
  }

  throw new Error('Failed to fetch project studies');
}

// 사용 예시
const data1 = await getProjectStudies(634); // 200 OK, ETag 저장
const data2 = await getProjectStudies(634); // 304 Not Modified (데이터 변경 없으면)
const data3 = await getProjectStudies(634, true); // 200 OK (강제 새로고침)
```
...
```http
GET /api/project-data/634/studies
...
### 1. **POST/DELETE 후 즉시 GET 시 캐시 무효화 필요**

```javascript
// ❌ 잘못된 방법
await assignStudyToProject();
const studies = await fetch('/api/project-data/634/studies'); // 캐시된 데이터 가능

// ✅ 올바른 방법
await assignStudyToProject();
const studies = await fetch('/api/project-data/634/studies', {
...
|----------|--------------|--------|
| 200 OK (첫 요청) | 0.053초 | - |
| 304 Not Modified | 0.015초 | **71.3% ⬆️** |
| 브라우저 캐시 (60초 내) | 0.000초 | **100% ⬆️** |

### **네트워크 절약**

- **200 OK**: ~10KB (Study 목록 데이터)
...
curl -i -H "Authorization: Bearer $TOKEN" \
  http://localhost:8080/api/project-data/634/studies
...
- [Study List View Cache Guide](./study-list-view-cache-client-guide.md) - Study List View 캐싱
...
| Study 할당 후 조회 | `Cache-Control: no-cache` 헤더 추가 |
