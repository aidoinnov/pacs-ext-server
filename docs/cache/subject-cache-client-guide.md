# Subject API - HTTP 캐싱 클라이언트 가이드

## 개요

Subject API는 **HTTP 캐싱 (ETag)**을 활용하여 성능을 최적화합니다.
...
- ✅ **네트워크 절약**: 304 응답 시 body 없음 (69.7% 응답 시간 단축)
...
### **시나리오 1: 일반적인 Subject 목록 조회**

브라우저가 자동으로 캐싱 처리.

```javascript
// 첫 번째 요청 - 200 OK, ETag 저장
const response1 = await fetch('/api/projects/634/subjects', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
const subjects1 = await response1.json();
console.log('ETag:', response1.headers.get('ETag')); // W/"1737734400"

// 60초 내 두 번째 요청 - 브라우저 캐시 사용 (0ms)
const response2 = await fetch('/api/projects/634/subjects', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
...
const response3 = await fetch('/api/projects/634/subjects', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
...
### **시나리오 2: Subject 추가/수정 후 즉시 조회**

Subject를 추가/수정한 후 즉시 최신 데이터를 조회해야 하는 경우.

#### ❌ **잘못된 방법** (캐시된 데이터 반환 가능)

```javascript
// Subject 추가
await fetch('/api/projects/634/subjects', {
  method: 'POST',
  headers: {
    'Authorization': `Bearer ${token}`,
    'Content-Type': 'application/json'
...
const response = await fetch('/api/projects/634/subjects', {
  headers: {
    'Authorization': `Bearer ${token}`
  }
});
// ❌ 새로 추가한 Subject가 목록에 없을 수 있음!
```
...
const subjects = await response.json(); // ✅ 최신 데이터 보장
```
...
async function getSubjects(projectId, forceRefresh = false) {
  const headers = {
    'Authorization': `Bearer ${token}`
  };
  ...
  const response = await fetch(`/api/projects/${projectId}/subjects`, { headers });

  if (response.status === 304) {
    console.log('데이터 변경 없음 - 캐시 사용');
    return cachedSubjects; // 캐시된 데이터 반환
  }
...
// 사용 예시
const data1 = await getSubjects(634); // 200 OK, ETag 저장
const data2 = await getSubjects(634); // 304 Not Modified (데이터 변경 없으면)
const data3 = await getSubjects(634, true); // 200 OK (강제 새로고침)
```
...
// ❌ 잘못된 방법
await createSubject();
const subjects = await fetch('/api/projects/634/subjects'); // 캐시된 데이터 가능

// ✅ 올바른 방법
await createSubject();
const subjects = await fetch('/api/projects/634/subjects', {
...
|----------|--------------|--------|
| 200 OK (첫 요청) | 0.051초 | - |
| 304 Not Modified | 0.016초 | **69.7% ⬆️** |
| 브라우저 캐시 (60초 내) | 0.000초 | **100% ⬆️** |

### **네트워크 절약**

- **200 OK**: ~5KB (Subject 목록 데이터)
...
curl -i -H "Authorization: Bearer $TOKEN" \
  http://localhost:8080/api/projects/634/subjects
...
| Subject 추가/수정 후 조회 | `Cache-Control: no-cache` 헤더 추가 |
