...
mod tests {
    ...
    fn test_openapi_servers_include_production() {
        ...
        for server in servers {
            ...
            if let Some(description) = &server.description {
                assert!(!description.is_empty(), "서버 설명은 비어있으면 안됩니다");
            }
        }
    }
    ...
    fn test_openapi_servers_descriptions() {
        ...
        let descriptions: Vec<&str> = servers
            .iter()
        ...
        assert!(
            descriptions
                .iter()
                .any(|desc| desc.contains("Local development")),
        ...
        assert!(
            descriptions
                .iter()
                .any(|desc| desc.contains("all interfaces")),
    ...
    fn test_openapi_servers_environment_coverage() {
        ...
        // 프로덕션 환경 커버리지
        let has_production = server_urls
            .iter()
            .any(|url| url.contains("api.pacs-server.com"));

        assert!(has_localhost, "로컬호스트 서버가 포함되어야 합니다");
        assert!(
            has_all_interfaces,
            "모든 인터페이스 서버가 포함되어야 합니다"
        );
