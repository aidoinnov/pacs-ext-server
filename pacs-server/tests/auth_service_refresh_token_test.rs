...
mock! {
    ...
    impl Clone for KeycloakClient {
        fn clone(&self) -> Self;
    }

    impl KeycloakClient {
        pub async fn refresh_access_token(&self, refresh_token: &str) -> Result<pacs_server::infrastructure::external::KeycloakTokenResponse, ServiceError>;
    }
}
...
async fn test_refresh_token_with_keycloak_success() {
    ...
    let mut mock_keycloak_client = MockKeycloakClient::new();
    ...
    let auth_service = AuthServiceImpl::new(
        mock_user_repo,
        jwt_service,
        Arc::new(mock_keycloak_client),
    );
    
    let refresh_token = "test-refresh-token";
    
    // When
    let result = auth_service.refresh_token_with_keycloak(refresh_token).await;
