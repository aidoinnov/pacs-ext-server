...
use pacs_server::application::dto::auth_dto::{LoginRequest, LoginResponse, VerifyTokenResponse, RefreshTokenRequest, RefreshTokenResponse};
...
async fn test_auth_use_case_refresh_token_invalid() {
    ...
    let result = auth_use_case.refresh_token(RefreshTokenRequest { refresh_token: "invalid_token".to_string() }).await;
    assert!(result.is_err());
}
...
async fn test_refresh_token_response_structure() {
    ...
    let refresh_response = auth_use_case.refresh_token(RefreshTokenRequest { refresh_token: token.to_string() }).await.unwrap();
