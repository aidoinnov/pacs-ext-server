#!/usr/bin/env python3
"""
Project Data Access API ETag 캐싱 E2E 테스트
...
3. 빈 목록 ETag 처리
4. 성능 측정
5. Cache-Control: no-cache 헤더 처리

Note: 데이터 변경 후 캐시 무효화 테스트는 Study 추가/삭제 API가 필요하므로 생략
"""
...
def test_project_studies_etag_cache(token, project_id=634):
    ...
    url = f"{BASE_URL}/api/project-data/{project_id}/studies"
    ...
    print(f"✅ Study 개수: {len(data1.get('studies', []))}")
...
def test_empty_list_etag(token):
    """테스트 3: 빈 목록 ETag 처리"""
    print(f"\n{'='*80}")
    print(f"테스트 3: 빈 목록 ETag 처리")
    print(f"{'='*80}")

    headers = {"Authorization": f"Bearer {token}"}

    # Study가 없을 가능성이 높은 프로젝트 ID 사용 (999999)
    empty_project_id = 999999
    url = f"{BASE_URL}/api/project-data/{empty_project_id}/studies"
    ...
    print("\n✅ 테스트 3 통과: 빈 목록도 ETag 캐싱 정상 작동")
    return True


def test_cache_performance(token, project_id=634):
    """테스트 4: 성능 측정 (304 응답이 더 빠른지 확인)"""
    print(f"\n{'='*80}")
    print(f"테스트 4: 캐시 성능 측정")
    print(f"{'='*80}")
...
def test_cache_invalidation_on_no_cache(token, project_id=634):
    """테스트 5: Cache-Control: no-cache 헤더 처리"""
    print(f"\n{'='*80}")
    print(f"테스트 5: Cache-Control: no-cache 헤더 처리")
    print(f"{'='*80}")
    ...
    print("\n✅ 테스트 5 통과: Cache-Control 헤더 처리 확인")
    return True


def test_concurrent_requests(token, project_id=634):
    """테스트 6: 동시 요청 처리"""
    print(f"\n{'='*80}")
...
def main():
    print("🚀 Project Data Access API ETag 캐싱 E2E 테스트 시작")
    print("="*80)
    ...
    # 테스트 스위트
    test_suite = [
        ("ETag 생성 및 304 응답", test_project_studies_etag_cache),
    ...
    if all(result for _, result, _ in results):
        print("\n🎉 모든 테스트 통과!")
        print("✅ Project Data Access API ETag 캐싱 기능 정상 작동")
        print("✅ 엣지 케이스 처리 정상")
        sys.exit(0)
    else:
        print("\n❌ 일부 테스트 실패")
