...
**엔드포인트**: `POST /api/v1/view-selections`
...
**엔드포인트**: `GET /api/v1/view-selections/{selection_id}`
...
**엔드포인트**: `DELETE /api/v1/view-selections/{selection_id}`
...
async function progressiveLoad(selectionId) {
  ...
  console.log('만료 시간:', selection.expires_at);
  ...
  console.log('연장된 만료 시간:', selection.expires_at);
  ...
  // 5. 다시 조회 → TTL 자동 연장
  selection = await getSelection(selectionId);
  console.log('다시 연장된 만료 시간:', selection.expires_at);
...
**의미**: `initial_views`를 사용하려면 `layout`도 함께 지정해야 합니다.
...
**의미**: Viewport 위치가 레이아웃 범위를 벗어났습니다. (2x2 레이아웃에서 (2, 3) 위치는 존재하지 않음)
...
**의미**: Series 목록이 비어있습니다. 최소 1개 이상의 Series가 필요합니다.
...
**의미**: Selection이 존재하지 않거나 만료되었습니다.
...
**제한사항**:
- ❌ 서버 재시작 시 모든 Selection 삭제됨
- ❌ 여러 서버 인스턴스 간 공유 불가 (로드 밸런싱 환경에서 문제 발생)
- ✅ 단일 서버 개발 환경에서는 사용 가능

**권장 사항**:
- 개발 환경: In-memory 사용 가능
- 프로덕션 환경: 반드시 Redis 사용
...
```javascript
// ✅ 좋은 예: Layout과 Initial Views를 함께 지정
{
  layout: { rows: 2, cols: 2 },
  initial_views: [
    { row: 0, col: 0, ... },
...
// ❌ 나쁜 예: Initial Views만 지정
{
  initial_views: [...]  // → 400 에러 발생
}
```
...
```javascript
// ✅ 좋은 예: Initial Views로 빠른 초기 렌더링
{
  layout: { rows: 2, cols: 2 },
  initial_views: [
    // 가장 중요한 이미지만 지정
    { row: 0, col: 0, study_uid: "...", series_uid: "...", sop_uid: "..." }
  ]
}
// → 나머지 Viewport는 Progressive Loading

// ❌ 나쁜 예: 모든 Viewport를 Initial Views로 지정
// → 초기 로딩 시간 증가
```
...
- **동시 사용자**: Redis 사용 시 제한 없음, In-memory 사용 시 단일 서버만 지원

## FAQ

### Q1. Selection이 만료되면 어떻게 되나요?

A: Selection이 만료되면 자동으로 삭제됩니다. GET 요청 시 404 에러가 반환됩니다.

### Q2. TTL을 연장하려면 어떻게 해야 하나요?

A: Selection을 조회(GET)하면 자동으로 TTL이 연장됩니다. 별도의 API 호출이 필요 없습니다.

### Q3. 여러 사용자가 같은 Selection을 공유할 수 있나요?

A: 네, Selection ID를 알고 있으면 누구나 조회할 수 있습니다. 단, JWT 토큰은 각자 필요합니다.

### Q4. In-memory 모드에서 서버를 재시작하면?

A: 모든 Selection이 삭제됩니다. 프로덕션 환경에서는 반드시 Redis를 사용하세요.

### Q5. Layout 없이 Initial Views만 사용할 수 있나요?

A: 아니요, Initial Views를 사용하려면 반드시 Layout도 함께 지정해야 합니다.

### Q6. 하나의 Viewport에 여러 이미지를 지정할 수 있나요?

A: 아니요, 각 Viewport는 하나의 이미지만 지정할 수 있습니다. 나머지는 Progressive Loading으로 처리하세요.
...
- **영문 가이드**: [VIEW_SELECTION_API_GUIDE.md](./VIEW_SELECTION_API_GUIDE.md)
