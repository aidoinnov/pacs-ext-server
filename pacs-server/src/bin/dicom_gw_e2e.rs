...
async fn main() {
    ...
    let access_token = if let (Some(u), Some(p)) = (username, password) {
        ...
        if let Some(sec) = client_secret.as_ref() {
    ...
    let studies_url = format!(
        "{}/api/dicom/studies?project_id=1&limit=1",
    ...
    if let Some(uid) = studies
        .as_array()
        .and_then(|a| a.get(0))
        .and_then(|item| item.get("0020000D"))
        ...
        .and_then(|v| v.as_str())
    {
        let series_url = format!(
            "{}/api/dicom/studies/{}/series?project_id=1&limit=1",
            gateway.trim_end_matches('/'),
            uid
        );
        let resp = http
        ...
        let series_uid = series
            .as_array()
            .and_then(|a| a.get(0))
            .and_then(|item| item.get("0020000E"))
            .and_then(|t| t.get("Value"))
            .and_then(|v| v.as_array())
            .and_then(|arr| arr.get(0))
            .and_then(|v| v.as_str());
        ...
        if let Some(sid) = series_uid {
            let instances_url = format!(
                "{}/api/dicom/studies/{}/series/{}/instances?project_id=1&limit=1",
                gateway.trim_end_matches('/'),
                uid,
                sid
            );
            let resp = http
                ...
                .send()
                .await
                .expect("instances request failed");
            if !resp.status().is_success() {
                let status = resp.status();
                let body = resp.text().await.unwrap_or_default();
            ...
            let instances: serde_json::Value =
                resp.json().await.expect("instances json parse failed");
        ...
        eprintln!(
            "No StudyInstanceUID found in /studies response to proceed with series/instances."
        );
    }
}
