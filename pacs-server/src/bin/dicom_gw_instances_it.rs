...
async fn main() {
    ...
    let server = HttpServer::new(move || {
        App::new().route(
            "/rs/studies/{study_uid}/instances",
            web::get().to(|| async {
    ...
    let app = actix_web::test::init_service(
        App::new().service(
            web::scope("/api/dicom")
                ...
                web::get().to(
                    pacs_server::presentation::controllers::dicom_gateway_controller::get_instances,
                ),
            ),
        ),
    )
    ...
    if !resp.status().is_success() {
        eprintln!(
            "Integration check (instances) failed: status={}",
            resp.status()
        );
        std::process::exit(1);
    }
    ...
    if !body.is_array() || body.as_array().unwrap().is_empty() {
        eprintln!(
            "Integration check (instances) failed: body not array or empty: {}",
            body
        );
        std::process::exit(2);
    }
    ...
    println!(
        "dicom_gw_instances_it OK: {} item(s)",
