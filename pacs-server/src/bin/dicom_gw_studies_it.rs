...
async fn main() {
    ...
    let app = actix_web::test::init_service(
        App::new().service(
            web::scope("/api/dicom")
                ...
                "/studies",
                web::get().to(
                    pacs_server::presentation::controllers::dicom_gateway_controller::get_studies,
                ),
            ),
        ),
    )
    ...
    if !resp.status().is_success() {
        eprintln!(
            "Integration check (studies) failed: status={}",
            resp.status()
        );
        std::process::exit(1);
    }
    ...
    if !body.is_array() || body.as_array().unwrap().is_empty() {
        eprintln!(
            "Integration check (studies) failed: body not array or empty: {}",
            body
        );
        std::process::exit(2);
    }
