...
where
    ...
    A: ProjectDataAccessRepository,
{
    async fn create_project_data(
        &self,
        new_data: NewProjectData,
    ) -> Result<ProjectData, ServiceError> {
            ...
            .map_err(|e| ServiceError::DatabaseError(e.to_string()))?
        {
            return Err(ServiceError::AlreadyExists(
                "Study already exists in project".to_string(),
            ));
        }
        ...
        self.grant_access_to_existing_users(project_data.id, new_data.project_id)
            .await?;
    ...
    async fn request_data_access(
        ...
        {
            return Err(ServiceError::AlreadyExists(
                "Access already exists".to_string(),
            ));
        }

        let new_access =
            NewProjectDataAccess::new(project_data_id, user_id, DataAccessStatus::Pending)
    ...
    async fn grant_default_access_to_user(
        ...
        for project_data in project_data_list {
            ...
            {
                let new_access =
                    NewProjectDataAccess::new(project_data.id, user_id, DataAccessStatus::Approved);
                let access = self
                    .project_data_access_repository
                    .create(&new_access)
                    .await
    ...
    async fn grant_access_to_existing_users(
        ...
        for (user_id,) in users {
            ...
            {
                let new_access =
                    NewProjectDataAccess::new(project_data_id, user_id, DataAccessStatus::Approved);
                let access = self
                    .project_data_access_repository
                    .create(&new_access)
                    .await
