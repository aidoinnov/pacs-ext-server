...
impl DicomRbacEvaluatorImpl {
    ...
    async fn get_study_dicom_values(&self, study_id: i32) -> StudyDicomValues {
        ...
        let result: Option<(Option<String>, Option<chrono::NaiveDate>, Option<String>)> =
            sqlx::query_as(
        ...
        if !project_conditions.is_empty() {
            for condition in project_conditions.iter() {
                ...
                let condition_level_str = match condition.resource_level {
                    ...
                    crate::domain::entities::access_condition::ResourceLevel::Instance => {
                        "INSTANCE"
                    }
                };
                        ...
                        ConditionType::Allow => {
                            if matches {
                                return RbacEvaluationResult {
                                    allowed: true,
                                    reason: Some(format!(
                                        "rule_based_allow: project_condition_{}",
                                        condition.id
                                    )),
                                };
                            }
                        ...
                        ConditionType::Deny => {
                            if matches {
                                return RbacEvaluationResult {
                                    allowed: false,
                                    reason: Some(format!(
                                        "rule_based_deny: project_condition_{}",
                                        condition.id
                                    )),
                                };
                            }
                        ...
                        ConditionType::Limit => {
                            ...
                            if matches {
                                return RbacEvaluationResult {
                                    allowed: true,
                                    reason: Some(format!(
                                        "rule_based_limit: project_condition_{}",
                                        condition.id
                                    )),
                                };
                            }
        ...
        if let Some(role_id) = self.get_user_role_id(user_id, project_id).await {
            ...
            if !role_conditions.is_empty() {
                for condition in role_conditions.iter() {
                    let condition_level_str = match condition.resource_level {
                        crate::domain::entities::access_condition::ResourceLevel::Study => "STUDY",
                        crate::domain::entities::access_condition::ResourceLevel::Series => {
                            "SERIES"
                        }
                        crate::domain::entities::access_condition::ResourceLevel::Instance => {
                            "INSTANCE"
                            ...
                            ConditionType::Allow => {
                                if matches {
                                    return RbacEvaluationResult {
                                        allowed: true,
                                        reason: Some(format!(
                                            "rule_based_allow: role_condition_{}",
                                            condition.id
                                        )),
                                    };
                                }
                            ...
                            ConditionType::Deny => {
                                if matches {
                                    return RbacEvaluationResult {
                                        allowed: false,
                                        reason: Some(format!(
                                            "rule_based_deny: role_condition_{}",
                                            condition.id
                                        )),
                                    };
                                }
                            ...
                            ConditionType::Limit => {
                                if matches {
                                    return RbacEvaluationResult {
                                        allowed: true,
                                        reason: Some(format!(
                                            "rule_based_limit: role_condition_{}",
                                            condition.id
                                        )),
                                    };
                                }
...
impl DicomRbacEvaluator for DicomRbacEvaluatorImpl {
    async fn evaluate_study_access(
        ...
        let data_inst: Option<i32> =
            sqlx::query_scalar("SELECT data_institution_id FROM project_data_study WHERE id = $1")
                .bind(study_id)
                .fetch_optional(&self.pool)
                .await
                .ok()
        ...
        if let (Some(u), Some(d)) = (user_inst, data_inst) {
            if u == d {
                return RbacEvaluationResult {
                    allowed: true,
                    reason: Some("same_institution".to_string()),
                };
            }
            ...
            if has_cross_access {
                return RbacEvaluationResult {
                    allowed: true,
                    reason: Some("institution_cross_access".to_string()),
                };
            }
        }
    ...
    async fn evaluate_instance_uid(
        ...
        if let Some(instance_id) = instance_id {
            return self
                .evaluate_instance_access(user_id, project_id, instance_id)
                .await;
        }
        RbacEvaluationResult {
            allowed: false,
...
mod tests {
    use super::*;
    use crate::domain::entities::access_condition::{ConditionType, ResourceLevel};
