...
use crate::domain::repositories::{AnnotationRepository, MaskGroupRepository, UserRepository};
...
pub trait MaskGroupService: Send + Sync {
    /// 새로운 마스크 그룹을 생성합니다.
    async fn create_mask_group(
        &self,
        new_mask_group: &NewMaskGroup,
    ...
    /// 마스크 그룹 통계를 조회합니다.
    async fn get_mask_group_stats(
        &self,
        annotation_id: Option<i32>,
    ) -> Result<MaskGroupStats, ServiceError>;
    ...
    async fn can_access_mask_group(
        &self,
        user_id: i32,
        mask_group_id: i32,
    ...
    async fn can_create_mask_group(
        &self,
        user_id: i32,
        annotation_id: i32,
    ) -> Result<bool, ServiceError>;
    ...
    UR: UserRepository + Send + Sync,
{
    async fn create_mask_group(
        ...
        println!(
            "🔍 [MaskGroupService] 어노테이션 존재 확인: annotation_id = {}",
        ...
        .await
        .map_err(|e| {
            ServiceError::DatabaseError(format!("Failed to check annotation existence: {}", e))
        })?;
        ...
        if let Some(created_by) = new_mask_group.created_by {
            ...
            .await
            .map_err(|e| {
                ServiceError::DatabaseError(format!("Failed to check user existence: {}", e))
            })?;
    ...
    async fn get_mask_group_by_id(&self, id: i32) -> Result<Option<MaskGroup>, ServiceError> {
        self.mask_group_repository.get_by_id(id).await
    }
    ...
    async fn delete_mask_group(&self, id: i32) -> Result<(), ServiceError> {
        ...
        // 삭제 실행
        self.mask_group_repository.delete(id).await
    }
    ...
    async fn list_mask_groups(
        ...
        if let Some(ann_id) = annotation_id {
            let annotation = self
                ...
                .await
                .map_err(|e| {
                    ServiceError::DatabaseError(format!("Failed to find annotation: {}", e))
                })?
                .ok_or_else(|| {
                    ServiceError::NotFound(format!("Annotation with id {} not found", ann_id))
                })?;
        }
    ...
    async fn get_masks_in_group(&self, mask_group_id: i32) -> Result<Vec<Mask>, ServiceError> {
        ...
        let mask_group = self
            ...
            .await?
            .ok_or_else(|| {
                ServiceError::NotFound(format!("Mask group with id {} not found", mask_group_id))
            })?;
    ...
    async fn get_mask_group_stats(
        ...
        self.mask_group_repository.get_stats(annotation_id).await
    }
    ...
    async fn can_access_mask_group(
        ...
        // 마스크 그룹 조회
        let mask_group = self.mask_group_repository.get_by_id(mask_group_id).await?;
        ...
        if annotation_owner || group_creator {
            ...
            Err(ServiceError::Unauthorized(
                "Access denied to mask group".to_string(),
            ))
        }
    }
    ...
    async fn can_create_mask_group(
        ...
        let annotation = self
            ...
            .ok_or_else(|| {
                ServiceError::NotFound(format!("Annotation with id {} not found", annotation_id))
            })?;
