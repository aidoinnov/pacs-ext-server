...
use crate::domain::repositories::{MaskGroupRepository, MaskRepository, UserRepository};
...
pub trait MaskService: Send + Sync {
    ...
    async fn can_create_mask(&self, user_id: i32, mask_group_id: i32)
    ...
    UR: UserRepository + Send + Sync,
{
    async fn create_mask(&self, new_mask: &NewMask) -> Result<Mask, ServiceError> {
        ...
        // 마스크 생성
        self.mask_repository.create(new_mask).await
    }

    async fn get_mask_by_id(&self, id: i32) -> Result<Option<Mask>, ServiceError> {
        self.mask_repository.get_by_id(id).await
    }

    async fn update_mask(&self, id: i32, update_mask: &UpdateMask) -> Result<Mask, ServiceError> {
        // 마스크가 존재하는지 확인
        let existing_mask = self
            .mask_repository
            .get_by_id(id)
            .await?
        ...
        // 업데이트 실행
        self.mask_repository.update(id, update_mask).await
    }
    ...
    async fn delete_mask(&self, id: i32) -> Result<(), ServiceError> {
        ...
        // 삭제 실행
        self.mask_repository.delete(id).await
    }
    ...
    async fn list_masks(
        ...
        if let Some(mg_id) = mask_group_id {
            let mask_group = self
                ...
                .await?
                .ok_or_else(|| {
                    ServiceError::NotFound(format!("Mask group with id {} not found", mg_id))
                })?;
        }
    ...
    async fn get_mask_stats(&self, mask_group_id: Option<i32>) -> Result<MaskStats, ServiceError> {
        ...
        self.mask_repository.get_stats(mask_group_id).await
    }
    ...
    async fn can_access_mask(&self, user_id: i32, mask_id: i32) -> Result<bool, ServiceError> {
        // 마스크 조회
        let mask = self.mask_repository.get_by_id(mask_id).await?;
