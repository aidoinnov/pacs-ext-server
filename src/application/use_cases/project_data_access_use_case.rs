...
impl ProjectDataAccessUseCase {
    ...
    pub async fn create_project_data(
        ...
        let new_data = NewProjectData::new(project_id, request.study_uid)
        ...
        if let Some(study_date_str) = request.study_date {
            if let Ok(study_date) = chrono::NaiveDate::parse_from_str(&study_date_str, "%Y-%m-%d") {
                let new_data = new_data.with_study_date(study_date);
                let _project_data = self.project_data_service.create_project_data(new_data).await?;
            }
        }
