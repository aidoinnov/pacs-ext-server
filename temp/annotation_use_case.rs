...
impl<A: AnnotationService, U: UserRepository, AC: AccessControlService> AnnotationUseCase<A, U, AC> {
    ...
    /// 권한 체크: 프로젝트 멤버이고, 본인 annotation이거나 READ_ALL 권한이 있어야 합니다.
    ///
    /// # 매개변수
    ...
    /// # 예시
    /// ```ignore
    /// let annotation = annotation_use_case.get_annotation_by_id(123, 1).await?;
    ...
    pub async fn get_annotation_by_id(
        ...
        let is_member = self
            .access_control_service
            .is_project_member(requesting_user_id, annotation.project_id)
            .await?;
        ...
        let is_owner = annotation.user_id == requesting_user_id;

        // 3. READ_ALL 권한 확인
        let has_read_all = self
            .access_control_service
            .check_permission(requesting_user_id, annotation.project_id, "ANNOTATION", "READ_ALL")
            .await?;

        // 4. 본인 것이 아니고 READ_ALL 권한도 없으면 접근 거부
        if !is_owner && !has_read_all {
            return Err(ServiceError::Unauthorized(
                "User can only read own annotations".into(),
            ));
        }
    ...
    /// 권한 체크: 프로젝트 멤버이고, 본인 annotation이어야 합니다.
    ///
    /// # 매개변수
    ...
    pub async fn update_annotation(
        ...
        let is_member = self
            .access_control_service
            .is_project_member(requesting_user_id, current_annotation.project_id)
            .await?;
        ...
        // 2. 본인 annotation인지 확인
        if current_annotation.user_id != requesting_user_id {
            return Err(ServiceError::Unauthorized(
    ...
    pub async fn delete_annotation(
        ...
        // 2. 본인 annotation인지 확인
        if annotation.user_id != requesting_user_id {
            return Err(ServiceError::Unauthorized(
